#!/usr/bin/env bash
#SBATCH --job-name=qwen35-08b-bc-vlm
...
dataset_root=/work/hdd/bgjs/mnakamura/robocasa/data_generation/task_l/evel/data/image/20260413T205634Z
...
if ! shards_match_pretokenized_defaults; then
  ...
  echo "Regenerate shards or update training/scripts/training_0.8B.sh to point at the intended shard set." >&2
  exit 1
fi
...
--run-name qwen35-08b-task-vlm-test \
-- \
--num-workers 8 \
